...
const generateSquares = () => shuffle(SQUARES).map((square) => (_jsx(motion.div, { layout: true, transition: { duration: 1.2, type: "spring" }, className: "h-full w-full overflow-hidden rounded-md bg-muted", style: {
        backgroundImage: `url(${square.src})`,
        backgroundSize: "cover",
        backgroundPosition: "center",
    } }, square.id)));
export function ShuffleGrid(props) {
    ...
    return (_jsx("div", { className: "grid h-[450px] grid-cols-4 grid-rows-4 gap-1", ...props, children: squares }));
}
export function ShuffleHero() {
    return (_jsxs("section", { className: "mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-8 px-8 py-12 md:grid-cols-2", children: [_jsxs("div", { className: "space-y-6", children: [_jsx("span", { className: "text-xs font-medium uppercase tracking-wide text-primary md:text-sm", children: "Better every day" }), _jsx("h3", { className: "text-4xl font-semibold text-foreground md:text-6xl", children: "Let's change it up a bit" }), _jsx("p", { className: "text-base text-muted-foreground md:text-lg", children: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nam nobis in error repellat voluptatibus ad." }), _jsx("button", { type: "button", className: cn("inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 font-medium text-primary-foreground transition-all", "hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"), children: "Find a class" })] }), _jsx(ShuffleGrid, {})] }));
}
