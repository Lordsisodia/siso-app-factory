...
import { Carousel, CarouselContent, CarouselItem, } from "@/components/1-menu-display/featured-carousels/carousel";
const data = [
    {
        id: "shadcn-ui",
...
const Gallery4 = ({ title = "Case Studies", description = "Discover how leading companies and developers are leveraging modern web technologies to build exceptional digital experiences. These case studies showcase real-world applications and success stories.", items = data, }) => {
    ...
    return (_jsxs("section", { className: "py-32", children: [_jsx("div", { className: "container mx-auto", children: _jsxs("div", { className: "mb-8 flex items-end justify-between md:mb-14 lg:mb-16", children: [_jsxs("div", { className: "flex flex-col gap-4", children: [_jsx("h2", { className: "text-3xl font-medium md:text-4xl lg:text-5xl", children: title }), _jsx("p", { className: "max-w-lg text-muted-foreground", children: description })] }), _jsxs("div", { className: "hidden shrink-0 gap-2 md:flex", children: [_jsx(Button, { size: "icon", variant: "ghost", onClick: () => {
                                        carouselApi?.scrollPrev();
                                    }, disabled: !canScrollPrev, className: "disabled:pointer-events-auto", children: _jsx(ArrowLeft, { className: "size-5" }) }), _jsx(Button, { size: "icon", variant: "ghost", onClick: () => {
                                        carouselApi?.scrollNext();
                                    }, disabled: !canScrollNext, className: "disabled:pointer-events-auto", children: _jsx(ArrowRight, { className: "size-5" }) })] })] }) }), _jsxs("div", { className: "w-full", children: [_jsx(Carousel, { setApi: setCarouselApi, opts: {
                            breakpoints: {
                                "(max-width: 768px)": {
                                    dragFree: true,
                                },
                            },
                        }, children: _jsx(CarouselContent, { className: "ml-0 2xl:ml-[max(8rem,calc(50vw-700px))] 2xl:mr-[max(0rem,calc(50vw-700px))]", children: items.map((item) => (_jsx(CarouselItem, { className: "max-w-[320px] pl-[20px] lg:max-w-[360px]", children: _jsx("a", { href: item.href, className: "group rounded-xl", children: _jsxs("div", { className: "group relative h-full min-h-[27rem] max-w-full overflow-hidden rounded-xl md:aspect-[5/4] lg:aspect-[16/9]", children: [_jsx("img", { src: item.image, alt: item.title, className: "absolute h-full w-full object-cover object-center transition-transform duration-300 group-hover:scale-105" }), _jsx("div", { className: "absolute inset-0 h-full bg-[linear-gradient(hsl(var(--primary)/0),hsl(var(--primary)/0.4),hsl(var(--primary)/0.8)_100%)] mix-blend-multiply" }), _jsxs("div", { className: "absolute inset-x-0 bottom-0 flex flex-col items-start p-6 text-primary-foreground md:p-8", children: [_jsx("div", { className: "mb-2 pt-4 text-xl font-semibold md:mb-3 md:pt-4 lg:pt-4", children: item.title }), _jsx("div", { className: "mb-8 line-clamp-2 md:mb-12 lg:mb-9", children: item.description }), _jsxs("div", { className: "flex items-center text-sm", children: ["Read more", " ", _jsx(ArrowRight, { className: "ml-2 size-5 transition-transform group-hover:translate-x-1" })] })] })] }) }) }, item.id))) }) }), _jsx("div", { className: "mt-8 flex justify-center gap-2", children: items.map((_, index) => (_jsx("button", { className: `h-2 w-2 rounded-full transition-colors ${currentSlide === index ? "bg-primary" : "bg-primary/20"}`, onClick: () => carouselApi?.scrollTo(index), "aria-label": `Go to slide ${index + 1}` }, index))) })] })] }));
};
export { Gallery4 };
