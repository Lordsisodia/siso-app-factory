...
import { useFeaturedDemos, useMainDemosExcludingFeatured, useLatestDemos, useTagDemos, getTagDemosCount, useLeaderboardDemosForHome, } from "@/lib/queries";
...
export function HomeTabLayout({ sortBy = "recommended" }) {
    ...
    const leaderboardItemOrderRef = useRef([]);
    ...
    useEffect(() => {
        ...
        if (isRandomizationDone && leaderboardItemOrderRef.current.length > 0) {
            ...
            const itemsMap = new Map(leaderboardDemosQuery.data.map((item) => [item.id, item]));
            ...
            const newItems = leaderboardDemosQuery.data.filter((item) => !existingIds.has(item.id));
        ...
        if (shouldRandomizeLeaderboard()) {
            ...
            leaderboardItemOrderRef.current = leaderboardDemosQuery.data.map((item) => item.id);
    ...
    const tagQueries = Object.fromEntries(tagCategories.map((category) => [
        category.id,
    ...
    const filteredPopularDemos = useMemo(() => {
        if (!popularDemosQuery.data)
            return [];
        if (!featuredDemosQuery.data?.ids)
            return popularDemosQuery.data;
    ...
    const handleVote = async (demoId) => {
        ...
        const demoIndex = randomizedLeaderboardItems.findIndex((demo) => demo.id === demoId);
        if (demoIndex === -1)
            return;
        ...
        try {
            ...
            queryClient.setQueryData(["leaderboard-demos-home", leaderboardDemosQuery.roundId], (oldData) => {
                ...
                return oldData.map((item) => {
                    if (item.id === demoId) {
                        return {
                            ...
                            votes_count: (item.votes_count || 0) + (currentVoteState ? -1 : 1),
                        };
                    }
                    return item;
                });
    ...
    return (_jsx("div", { className: "space-y-8", children: sliderGroups.map((group) => (_jsx(HorizontalSlider, { title: group.title, items: group.items || [], isLoading: group.isLoading, onViewAll: () => handleViewAll(group), totalCount: group.totalCount, viewAllUrl: group.viewAllUrl, isLeaderboard: group.isLeaderboard, onVote: group.isLeaderboard ? handleVote : undefined }, group.id))) }));
}
