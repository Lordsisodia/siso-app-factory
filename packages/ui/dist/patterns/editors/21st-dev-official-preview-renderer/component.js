...
import { SandpackProvider as SandpackProviderUnstyled, SandpackPreview, } from "@codesandbox/sandpack-react";
...
export function PreviewRenderer({ isDarkTheme, providerProps, component, code, demoCode, dependencies, demoDependencies, demoComponentNames, registryDependencies, npmDependenciesOfRegistryDependencies, tailwindConfig, globalCss, demo, css, shellCode, allDependencies, }) {
    ...
    if (shouldShowErrorState) {
        ...
        displayContent = (_jsxs(_Fragment, { children: [contentLoading && _jsx(LoadingOverlay, { text: getCurrentLoadingMessage() }), _jsx("iframe", { src: isDarkTheme ? `${bundle.html}?dark=true` : bundle.html, className: "w-full h-full", onLoad: () => setContentLoading(false), onError: () => {
                        setContentError(true);
                        setContentLoading(false);
                    } })] }));
    }
