...
export const EditorCodePanel = React.memo(function EditorCodePanel({ onCodeChange, componentPath, }) {
    const { code } = useActiveCode();
    const { sandpack } = useSandpack();
    const prevCodeRef = useRef(undefined);
    const updatingRef = useRef(false);
    const initialCodeRef = useRef(null);
    const { markFileAsResolved, isActionRequired, getActionDetails } = useActionRequired();
    ...
    const normalizedPath = useMemo(() => componentPath.replace(/^@\//, "/"), [componentPath]);
    // Check if this is an unknown component that needs action - memoize this value
    const isUnresolvedDependency = useMemo(() => codeManager?.isUnresolvedDependency?.(componentPath) || false, [codeManager, componentPath]);
    ...
    const handleFileMissing = useCallback(() => {
        ...
        if (cachedContent) {
            try {
                console.log("[EditorCodePanel] Restoring file from cache:", normalizedPath);
            ...
            try {
                ...
                console.log("[EditorCodePanel] File created and cached:", normalizedPath);
            }
            catch (error) {
    ...
    const handleCodeUpdate = useCallback(() => {
        ...
        if (sandpack.activeFile === normalizedPath) {
            ...
            const checkAndResolveActionRequired = (path) => {
    ...
    return _jsx(MemoizedSandpackCodeEditor, { ...editorProps });
});
export function SimpleEditor({ onChange }) {
    ...
    return (_jsx(SandpackCodeEditor, { showTabs: false, showLineNumbers: true, showInlineErrors: true, className: "h-full", style: { height: "100%" } }));
}
