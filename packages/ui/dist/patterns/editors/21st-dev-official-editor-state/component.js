...
// Import atoms
import { activeFileAtom, userModifiedFilesAtom, loadingComponentsAtom, } from "./editor-atoms";
// Import hooks
import { useActionRequired } from "../hooks/use-action-required";
import { usePreviewReady } from "../hooks/use-preview-ready";
// Create the context
const CodeManagerContext = createContext(null);
/**
 ...
 */
export function CodeManagerProvider({ children, initialComponentPath, unresolvedDependencies = [], onFileContentChange, isUnknownComponentFn = () => false, }) {
    ...
    const [userModifiedFiles, setUserModifiedFiles] = useAtom(userModifiedFilesAtom);
    const [loadingComponents, setLoadingComponents] = useAtom(loadingComponentsAtom);
    // Get action required functions from the hook instead of redefining them
    const { actionRequiredFiles, actionRequiredPaths, markFileAsRequiringAction, markFileAsResolved, isActionRequired, getActionDetails, } = useActionRequired();
    // Get preview ready functions from the hook
    const { previewReady, markPreviewReady, markPreviewNotReady } = usePreviewReady();
    // Watch for demo.tsx file changes to automatically set preview ready state
    useEffect(() => {
        if (!sandpack)
            return; // Skip if outside of SandpackProvider
    ...
    const selectFile = (path) => {
        ...
        if (!isUnknownComponentFn(path) && sandpack) {
            ...
            console.log("[CodeManager] Unknown component - not setting in Sandpack:", path);
        }
        // Update atom state
        setActiveFile(path);
    ...
    const getComponentName = (path) => {
        ...
        if (actionDetails?.reason === "unresolved_dependencies" &&
    ...
    return (_jsx(CodeManagerContext.Provider, { value: value, children: children }));
}
