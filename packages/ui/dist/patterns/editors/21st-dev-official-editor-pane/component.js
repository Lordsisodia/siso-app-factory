...
const activateMonacoJSXHighlighter = async (monacoEditor, monaco) => {
    ...
    const { default: MonacoJSXHighlighter, JSXTypes, makeBabelParse, //By @HaimCandiTech
     } = await import(
    // @ts-ignore
    ...
    console.log("Customize each JSX expression type's options, they must match monaco.editor.IModelDecorationOptions:", JSXTypes);
...
function EditorPaneOriginal({ selectedFile, code, onCodeChange, isLoading, }) {
    const { resolvedTheme } = useTheme();
    const handleEditorMount = (editor, monaco) => {
        const compilerOpts = monaco.languages.typescript.typescriptDefaults.getCompilerOptions();
    ...
    if (isLoading) {
        return (_jsx("div", { className: "h-full flex items-center justify-center", children: _jsx(Loader2Icon, { className: "h-8 w-8 animate-spin text-primary" }) }));
    }
    if (!selectedFile || selectedFile.type !== "file") {
        return (_jsx("div", { className: "h-full flex items-center justify-center text-muted-foreground", children: selectedFile && selectedFile.type === "dir"
                ? `Directory: ${selectedFile.path}`
                : "Select a file to start editing" }));
    }
    return (_jsxs("div", { className: "h-full flex flex-col", children: [isReadOnly && (_jsx("div", { className: "p-2 bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300 text-xs text-center", children: "This component is from the 21st.dev registry and is read-only." })), _jsx(Editor, { height: "100%", language: getMonacoLanguage(selectedFile.path), value: code, onChange: (value) => onCodeChange(value || ""), theme: resolvedTheme === "dark" ? "vs-dark" : "vs", className: "oveflow-hidden grow", options: {
                    ...
                    readOnly: isReadOnly,
                }, onMount: handleEditorMount, loading: _jsx("div", { className: "h-full flex items-center justify-center", children: _jsx(Loader2Icon, { className: "h-6 w-6 animate-spin text-primary" }) }) })] }));
}
