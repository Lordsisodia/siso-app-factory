...
function generateCatalog() {
  ...
  const searchRoots = [
    { scope: 'platform', dir: path.join(repoRoot, 'packages/platform') },
    { scope: 'experiences', dir: path.join(repoRoot, 'packages/experiences') },
    { scope: 'templates', dir: path.join(repoRoot, 'templates') }
  ]

  const metadataFiles = []

  searchRoots.forEach(({ scope, dir }) => {
    if (!fs.existsSync(dir)) return
    const discovered = findMetadataFiles(dir).map(file => ({
      file,
      scope,
      baseDir: dir
    }))
    metadataFiles.push(...discovered)
  })

  console.log(`Found ${metadataFiles.length} metadata files across platform/experiences/templates:\n`)
  metadataFiles.forEach(entry => {
    console.log(`  ✅ [${entry.scope}] ${path.relative(repoRoot, entry.file)}`)
  })
  ...
  metadataFiles.forEach(entry => {
    ...
    const relativePath = path.relative(entry.baseDir, path.dirname(entry.file))
    const repoRelativePath = path.relative(repoRoot, path.dirname(entry.file))
    const parts = relativePath.split(path.sep)

    // Determine if this is a UI component or feature
    // Path format: ui/src/primitives/buttons/solid-button
    const isUIComponent = entry.scope === 'platform' && parts[0] === 'ui' && parts[2] === 'primitives'
    const isTemplateFeature = entry.scope === 'templates' && parts.includes('features')
    const isExperience = entry.scope === 'experiences'
    ...
    if (isUIComponent) {
    ...
    } else if (isTemplateFeature) {
      // Feature
      const domain = parts[0] // e.g., "bike-rental", "tour-guides"
      ...
      catalog.experience_count++

      const domain = metadata.domain || 'experiences'
      buildFeatureIndices(metadata, catalog.search_index, domain)
    }
  })

  // Write catalog
  const catalogPath = path.join(__dirname, '../../docs/catalog/components-and-features.json')
  fs.writeFileSync(catalogPath, JSON.stringify(catalog, null, 2))

  console.log('\n📊 Catalog Generated:\n')
  console.log(`  Component count: ${catalog.component_count}`)
  console.log(`  Feature count: ${catalog.feature_count}`)
  console.log(`  Experience count: ${catalog.experience_count}`)
  console.log(`  UI component types: ${Object.keys(catalog.ui_components).length}`)
  console.log(`  Template domains: ${Object.keys(catalog.features).length}`)
  console.log(`\n  Search indices:`)
