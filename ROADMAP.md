...
├── docs/
│   └── catalog/components-and-features.json  # Master AI-searchable catalog
└── tools/
    ├── generate-metadata.cjs  # Auto-catalog builder
    └── ai-search.cjs          # AI search engine
```
...
**File:** `tools/core/component-import/bulk-import.cjs`

**What it does:**
1. Reads source library folder
2. Detects component type (button, accordion, card, etc.)
3. Copies to `packages/platform/ui/src/primitives/{type}/{variant-name}/`
...
node tools/core/component-import/bulk-import.cjs --source magic-ui --category accordions
```
...
```
packages/platform/ui/src/primitives/{type}/{source}-{variant}/
                          ↓      ↓        ↓
...
const catalog = require('./docs/catalog/components-and-features.json')
...
cat docs/catalog/components-and-features.json | grep "component_count"
# Shows current count
```
...
1. **If component_count < 100:**
   ...
   - Use tools/core/component-import/bulk-import.cjs (if created) or create it
...
- `docs/catalog/components-and-features.json` - Current state of catalog
- `tools/core/generate-metadata.cjs` - Catalog generator
- `tools/core/ai-search.cjs` - AI search tool
...
cat docs/catalog/components-and-features.json | jq '.component_count'

# Check feature count
cat docs/catalog/components-and-features.json | jq '.feature_count'
```
...
- [ ] catalog/components-and-features.json complete
...
3. **Check catalog/components-and-features.json** - See what's already done
...
│
├── docs/
│   ├── catalog/components-and-features.json                        # THE MASTER CATALOG
│   ├── imports/                               # Staging area
│   ├── USAGE.md                               # How AI uses this
│   └── COMPONENT-REFERENCE.md                 # Quick reference
│
└── tools/
    ├── generate-metadata.cjs                   # Catalog builder ✅
    ├── ai-search.cjs                           # Search engine ✅
    ├── bulk-import.cjs                         # Bulk importer (to build)
...
**Total Imported:** 650/3000+ (~22% complete)
...
✅ Catalog written to: docs/catalog/components-and-features.json (8.4 MB)
```
