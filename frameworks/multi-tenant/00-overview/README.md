...
This framework now follows a numbered layout:

1. **`00-overview/`** – positioning, concept diagrams, and this README.
2. **`10-architecture/`** – high-level + data architecture (see `../10-architecture/data-architecture.md`).
3. **`20-platform/`** – runtime contracts:
   - `database/` → schema + RLS (`MULTI-TENANT-SCHEMA.md`).
   - `theming/` → token system and presets (`CONFIG-DRIVEN-THEMING.md`, `theme-tokens.md`).
   - `feature-flags/` → config model for enabling pages/features (`FEATURE-FLAGS.md`).
4. **`30-deployment/`** – fork/deploy/ops docs (`FORK-AND-DEPLOY.md`, env checklists).
5. **`40-playbooks/`** – action-ready guides like `NEW-CLIENT-SETUP.md`.
6. **`90-assets/`** – reserved for SQL/scripts referenced in docs (add as they’re created).

Starter templates will live under `frameworks/templates/` once published; until then the playbooks reference generic GitHub repos you can fork.
Shared SQL/scripts that multiple frameworks need should sit in `../../shared-assets/` to avoid duplication; reference them from `90-assets/` as needed.
...
1. `../20-platform/database/MULTI-TENANT-SCHEMA.md` – database design with `client_id` everywhere.
2. `../20-platform/database/RLS-POLICIES.md` – **TODO:** detail Supabase RLS policies (placeholder).
3. `../20-platform/theming/CONFIG-DRIVEN-THEMING.md` – how dynamic theming works.
4. `../20-platform/theming/theme-tokens.md` – token schema reference.
5. `../20-platform/feature-flags/FEATURE-FLAGS.md` – config-as-data + variant registry.
6. `../30-deployment/FORK-AND-DEPLOY.md` – deployment walkthrough + env vars.
7. `../40-playbooks/NEW-CLIENT-SETUP.md` – 10-minute setup guide.
8. `frameworks/templates/...` – (coming soon) starter repos to fork per vertical.
...
**Start with:** `../40-playbooks/NEW-CLIENT-SETUP.md`
