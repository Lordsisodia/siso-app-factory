...
> **Path Update (Nov 20, 2025):** References to `packages/ui/...` now map to `packages/platform/ui/...`, and template folders moved to `templates/*`.
...
- **650+ UI Components** - Buttons, cards, inputs, modals, forms, tables, etc.
- **12 Complete Features** - Booking systems, admin panels, payment processing, etc.
- **AI-Powered Catalog** - Single file (`docs/catalog/components-and-features.json`) with everything indexed
...
const { searchComponents } = require('./tools/core/ai-search.cjs')
...
5. **File location** (if they want to customize)
   - `packages/platform/ui/src/primitives/buttons/solid-button/component.tsx`
...
```javascript
const { searchFeatures } = require('./tools/core/ai-search.cjs')
...
# Check stats
cat docs/catalog/components-and-features.json | jq '.component_count, .feature_count'

# List all component types
cat docs/catalog/components-and-features.json | jq '.ui_components | keys'

# List all features
cat docs/catalog/components-and-features.json | jq '.features'

# Search by tag
cat docs/catalog/components-and-features.json | jq '.search_index.by_tag.cta'

# Get component by ID
cat docs/catalog/components-and-features.json | jq '.ui_components.buttons.variations[] | select(.id == "solid-button")'
```
...
```
packages/platform/ui/src/primitives/{type}/{component-id}/
